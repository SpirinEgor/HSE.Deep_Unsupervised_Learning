...
class WGanGradientPolicy:
    ...
    def train(
        ...
        n_epochs = n_critic_step * n_iterations // len(train_dataloader)
        ...
        epoch_bar = trange(n_epochs, desc="Train")
        postfix = {}
        losses = []
        for _ in epoch_bar:
            for i, batch in enumerate(train_dataloader):
                ...
                mean_fake_data = self._discriminator(fake_data).mean()
                d_loss = mean_fake_data - self._discriminator(batch).mean() + self._gp_cf * gp
                d_optim.zero_grad()
                d_loss.backward()
                d_optim.step()
    ...
    def sample(self, n_samples: int) -> Tensor:
        ...
        with torch.no_grad():
            return self._generator.sample(n_samples).detach().cpu().numpy()
