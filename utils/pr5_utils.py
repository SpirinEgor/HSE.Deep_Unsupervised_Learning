...
def sample_four_blobs(n):
    ...
    x, _ = make_blobs(n, n_features=2, centers=centers, cluster_std=st_devs, shuffle=True)
    return x.astype("float32")
...
def sample_cov_gauss_data(count):
    ...
    return (
        [[1.0, 2.0]]
        + (rand.randn(count, 2) * [[5.0, 1.0]]).dot(
...
def plot_scatter_2d(points, title="", labels=None):
    ...
    if labels is not None:
        plt.scatter(
            points[:, 0], points[:, 1], c=labels, cmap=mpl.colors.ListedColormap(["red", "blue", "green", "purple"])
        )
    else:
